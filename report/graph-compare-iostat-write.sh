...
for SINGLE_KERNEL in $KERNEL; do
	...
	if [ $? -eq 0 ]; then
		TIMESTAMPS=yes
		cat iostat-$SINGLE_KERNEL-* | grep -- "-- $IOSTAT_DEVICE " | awk "{print (\$1-$START)\" \"\$11}" > $TMPDIR/iostat-wbs-$NAME-$SINGLE_KERNEL.data-unsorted
	else
		echo -n > $TMPDIR/iostat-wbs-$NAME-$SINGLE_KERNEL.data-unsorted
		for TEST in $MMTESTS; do
			zcat iostat-$SINGLE_KERNEL-$TEST.gz | grep "^$IOSTAT_DEVICE " | awk "{print ($COUNT+NR)\" \"\$7}" >> $TMPDIR/iostat-wbs-$NAME-$SINGLE_KERNEL.data-unsorted
			THISCOUNT=`zcat iostat-$SINGLE_KERNEL-$TEST.gz | wc -l`
			COUNT=$(($COUNT+$THISCOUNT))
		done
	fi
	sort -n $TMPDIR/iostat-wbs-$NAME-$SINGLE_KERNEL.data-unsorted > $TMPDIR/iostat-wbs-$NAME-$SINGLE_KERNEL.data
	rm $TMPDIR/iostat-wbs-$NAME-$SINGLE_KERNEL.data-unsorted
...
PLOTS=
for SINGLE_KERNEL in $KERNEL; do
	PLOTS="$PLOTS $TMPDIR/iostat-wbs-$NAME-$SINGLE_KERNEL.data"
done
...
$PLOT \
	...
	--titles $TITLES \
	--output $OUTPUTDIR/iostat-wbs-comparison-$NAME.ps \
	$PLOTS
echo Generated iostat-wbs-comparison-$NAME.ps
...
$PLOT \
	...
	--ylabel "Sectors per Second" \
	--titles $TITLES \
	--output $OUTPUTDIR/iostat-wbs-comparison-smooth-$NAME.ps \
	$PLOTS
echo Generated iostat-wbs-comparison-smooth-$NAME.ps
