...
sub extractReport($$$) {
	...
	foreach my $wl (@workloads) {
		...
		my @files = <$reportDir/noprofile/filelockperf-$wl-1-$type-*.log>;
		...
		foreach my $nthr (@threads) {
			foreach my $type (@locktypes) {
				...
				foreach my $iter (1..$iterations) {
					my $file = "$reportDir/noprofile/filelockperf-$wl-$iter-$type-$nthr.log";
