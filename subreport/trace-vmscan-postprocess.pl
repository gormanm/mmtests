...
use constant MM_VMSCAN_WRITEPAGE_MIXED_SYNC	=> 12;
use constant MM_VMSCAN_WRITEPAGE_MIXED_ASYNC	=> 13;
use constant MM_VMSCAN_WRITEPAGE		=> 14;
use constant EVENT_UNKNOWN			=> 15;
...
my $regex_traceevent = '\s*([a-zA-Z0-9-]*)\s*(\[[0-9]*\])\s*[0-9.]*\s*([0-9.]*):\s*([a-zA-Z_]*):\s*(.*)';
...
EVENT_PROCESS:
	while ($traceevent = <STDIN>) {
		...
		if ($tracepoint eq "mm_vmscan_direct_reclaim_begin") {
			...
			my $flags = $3;
			$perprocesspid{$process_pid}->{MM_VMSCAN_WRITEPAGE}++;
			...
			if ($flags =~ /RECLAIM_WB_MIXED/) {
				if ($flags =~ /RECLAIM_WB_ASYNC/) {
					$perprocesspid{$process_pid}->{MM_VMSCAN_WRITEPAGE_MIXED_ASYNC}++;
				} else {
					$perprocesspid{$process_pid}->{MM_VMSCAN_WRITEPAGE_MIXED_SYNC}++;
				}
			}
		} else {
...
sub dump_stats {
	...
	foreach $process_pid (keys %stats) {
		...
		if (!$stats{$process_pid}->{MM_VMSCAN_DIRECT_RECLAIM_BEGIN} &&
				!$stats{$process_pid}->{MM_VMSCAN_WRITEPAGE}) {
			next;
		}
		next if ($process_pid =~ /kswapd[0-9]*/);
