...
# Test Case 2: Basic patch/unpatch of getppid

set -e
. $(dirname $0)/klp_tc_functions.sh
klp_tc_init "Test Case 2: Basic patch/unpatch of getppid"

klp_tc_milestone "Compiling kernel live patch"
PATCH_KO="$(klp_create_patch_module tc_2 ${KLP_TEST_SYSCALL_FN_PREFIX}sys_getppid)"
PATCH_MOD_NAME="$(basename "$PATCH_KO" .ko)"

klp_tc_milestone "Inserting getppid patch"
insmod "$PATCH_KO"
