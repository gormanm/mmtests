	...
	*)
	...
	if [ "$FORMAT" = "html" -a -d "$OUTPUT_DIRECTORY" ]; then
		...
		wptlbflush)
			...
			eval $GRAPH_PNG        -b $SUBREPORT --title \"$SUBREPORT sorted\" --output $OUTPUT_DIRECTORY/graph-${SUBREPORT}-sorted.png --sort-samples-reverse
			eval $GRAPH_PSC        -b $SUBREPORT --title \"$SUBREPORT sorted\" --output $OUTPUT_DIRECTORY/graph-${SUBREPORT}-sorted.ps --sort-samples-reverse
